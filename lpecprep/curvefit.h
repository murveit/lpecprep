...
class CurveFitting
{
    public:
        ...
        void fitCurve(const FittingGoal goal, const QVector<double> &position, const QVector<double> &hfr,
                        ...
                        const StarParams &starParams, const CurveFit curveFit, const bool useWeights)
        {
            ...
            {
                qDebug() << QString("CurveFitting::fitCurve3D null image ptr");
            ...
            if (imageWidth <= 0)
            {
                qDebug() << QString("CurveFitting::fitCurve3D imageWidth=%1").arg(imageWidth);
                m_FirstSolverRun = true;
                return;
            }
            ...
            if (useWeights)
            {
                qDebug() << QString("CurveFitting::fitCurve3D called with useWeights. Not yet implemented");
                m_FirstSolverRun = true;
                return;
            }
            ...
            switch (m_CurveType)
            {
                ...
                default :
                    ...
                    qDebug() << QString("CurveFitting::fitCurve3D called with curveFit=%1").arg(curveFit);
                    m_FirstSolverRun = true;
                    return;
            }
